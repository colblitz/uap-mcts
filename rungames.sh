...
function runGames() {
  writeToConfig black $1 $3
  writeToConfig white $2 $3
  setPlayers black white
  getNewNum

  SGFDir="$Results/$NewNum"
  echo "Results file = $SGFDir"
  echo "Running $NuGames $Size x $Size games with time $Time"
  time1=$(date +%s.%N)
  time gogui-twogtp -black "$BPlayer" -white "$WPlayer" -alternate -games $NuGames -sgffile $SGFDir -size $Size -komi 7.5 -auto -time $Time $Verbose -debugtocomment
  time2=$(date +%s.%N)
